...
vars <- read.csv("ASXL1_variants.csv")
vars$Patient <- as.factor(vars$Patient)
ggplot(vars, aes(AF, CADD, color = Disease_Status, shape = Patient, label = AA)) +
    geom_point(size = 3) + 
    theme_classic() +
    scale_color_manual(values = c("Bohring-Opitz syndrome" = "dark gray", 
                                  "AML/MDS" = "navy blue",
                                  "Patient" ="#550307FF", 
                                  "Benign" = "#4B644BFF")) +
    ...
    labs(y = "Allelic Frequency (gnomAD)", x = "Predicted CADD Score") 
```
...
BloodPlot(mt = plt.pbmc, 
          vartype = "categorical", 
          varname = "Disease_Status", 
          tissue = "bloodextended") #+ scale_fill_manual(values = group.colors) NK
...
cairo_pdf(filename = paste0("Figures_2/", Sys.Date(), "_Horvath_PBMC.pdf"),
              width = 5.5, height = 4.5, onefile = TRUE)
ggplot(meta[meta$Cell_Type == "PBMC", ], aes(Age, Horvath)) +  # Whole Blood
    geom_smooth(method = "lm") + 
    geom_point(size = 3, alpha = 0.7, aes(color = Disease_Status)) + 
    theme_classic() + 
    scale_color_manual(values = c("#A31D4B", "#5E81ACFF")) + 
    ylim(0, 28) +
    xlim(0, 20) +
    theme(text = element_text(size = 16))
dev.off()
cairo_pdf(filename = paste0("Figures_2/", Sys.Date(), "_TL_WB.pdf"),
              width = 5.5, height = 4.5, onefile = TRUE)
ggplot(meta[meta$Cell_Type == "Whole Blood", ], aes(Age, TL)) + 
    geom_smooth(method = "lm") + 
    geom_point(size = 3, alpha = 0.7, aes(color = Disease_Status)) + 
    theme_classic() + 
    scale_color_manual(values = c("#A31D4B", "#5E81ACFF")) + 
...
ASXL1.test.wb$meandiff <- mean(c(ASXL1.test.wb$betas.pat, ASXL1.test.wb$betas.pat_pbmc.r) - ASXL1.test.wb$betas.refM)
...
skewtest.pbmc <- skewness.norm.test(limmaOut.pbmc$logFC)
skewtest.wb <- skewness.norm.test(limmaOut.wb$logFC)
```
...
makeVolcano2(name = rownames(limmaOut.wb), 
             ...
             deltabeta = limmaOut.wb$logFC*100, 
             ymax = 110, 
             alpha = 0.8, 
             palette = 5, 
             xmin = -100, 
...
makeVolcano2(name = rownames(limmaOut.pbmc), 
             ...
             deltabeta = limmaOut.pbmc$logFC*100, 
             ymax = 191, 
             alpha = 0.8, 
             palette = 5, 
             xmin = -100, 
...
cpg <- "cg09007841"
plt <- beta.BC[rownames(beta.BC) %in% rownames(fdat_EPIC_Mreg)[fdat_EPIC_Mreg$TargetID == cpg], rownames(meta)[meta$Cell_Type == "PBMC"]] %>%
    melt(.) %>%
    cbind(., fdat_EPIC_Mreg[cpg, ]) %>% 
    merge(., meta, by = 0)
ggplot(plt, aes(TargetID, value, color = Disease_Status, group = Disease_Status)) + 
    geom_violin(position = position_identity(), width = 0.4) + 
    geom_point() +
    geom_line() + 
    theme_bw() + 
    scale_color_manual(values = c("Healthy Control" = "#ADBFD5", 
                         "ASXL1 variant" = "#A31D4B")) +
    theme(axis.text = element_text(size = 12, color = "black"),
              axis.title = element_text(size = 12),
              legend.text = element_text(size = 12),
              legend.title = element_text(size = 12)) +
    labs(x = "", y = "DNA methylation level (%)")

```

##### Overlap - WB and PBMC hits
...
# Subset to hits
limmaHits.wb <- limmaOut.wb[limmaOut.wb$adj.P.Val <= cutoff.pv & abs(limmaOut.wb$logFC) >= cutoff.db, ]
limmaHits.pbmc <- limmaOut.pbmc[limmaOut.pbmc$adj.P.Val <= cutoff.pv & abs(limmaOut.pbmc$logFC) >= cutoff.db, ]
...
limmaHits.O <- list(WB = rownames(limmaHits.wb),
                    PBMC = rownames(limmaHits.pbmc))
...
Chrom_df <- EPIC_Anno_ChromeHMM[rownames(limmaOut.wb), c("Name", "E062_Mononuclear_State")]
...
save(res_cedar2, file = "RData/13-Cedar_ct_specific_DMs.RData")
...
group.colors <- c("Healthy Control" = "#4B644BFF", 
                  "ASXL1 variant" = "#550307FF")
...
ggplot(plt, aes(Disease_Status, DNAm, color= Disease_Status)) +
    ...
    geom_boxplot(width = 0.3, alpha = 0.8, fill = "white") +
...
(gene <- fdat_EPIC_Mreg[grep("NCAM1", fdat_EPIC_Mreg$UCSC_REFGENE_NAME), 
                        c("CHR", "MAPINFO", "STRAND", "UCSC_REFGENE_NAME", "UCSC_REFGENE_GROUP"), ])
(gene <- fdat_EPIC_Mreg[grep("RAB27A", fdat_EPIC_Mreg$UCSC_REFGENE_NAME), 
                        c("CHR", "MAPINFO", "STRAND", "UCSC_REFGENE_NAME", "UCSC_REFGENE_GROUP"), ])
(gene <- fdat_EPIC_Mreg[grep("UNC13D", fdat_EPIC_Mreg$UCSC_REFGENE_NAME), 
                        c("CHR", "MAPINFO", "STRAND", "UCSC_REFGENE_NAME", "UCSC_REFGENE_GROUP"), ])
(gene <- fdat_EPIC_Mreg[grep("PRF1", fdat_EPIC_Mreg$UCSC_REFGENE_NAME), 
                        c("CHR", "MAPINFO", "STRAND", "UCSC_REFGENE_NAME", "UCSC_REFGENE_GROUP"), ])
(gene <- fdat_EPIC_Mreg[grep("B3GAT1", fdat_EPIC_Mreg$UCSC_REFGENE_NAME), 
                        c("CHR", "MAPINFO", "STRAND", "UCSC_REFGENE_NAME", "UCSC_REFGENE_GROUP"), ])
(gene <- fdat_EPIC_Mreg[grep("ITGAL", fdat_EPIC_Mreg$UCSC_REFGENE_NAME), 
                        c("CHR", "MAPINFO", "STRAND", "UCSC_REFGENE_NAME", "UCSC_REFGENE_GROUP"), ])

  
cpgs <- rownames(gene)[rownames(gene) %in% rownames(beta.t)]

plt <- beta.t[cpgs, ] %>%
    as.data.frame(.)
plt$mapinfo <- fdat_EPIC_Mreg[cpgs, "MAPINFO"] %>% as.character(.)
plt$Padj <- limmaOut.pbmc[cpgs, "adj.P.Val"]
plt <- reshape2::melt(plt, id.vars = c("mapinfo"))
...
ggplot(plt, aes(x = mapinfo, y = value, color = Disease_Status, shape = Disease_Status, alpha = Disease_Status)) + 
    ...
    scale_color_manual(values = c("Healthy Control" =  "#5E81ACFF", 
                                  "ASXL1 variant" = "#40346A")) + 
    scale_shape_manual(values = c("Healthy Control" = 16, 
                                  "ASXL1 variant" = 17)) +
    scale_size_manual(values = c("Healthy Control" = 1.2, 
                                 "ASXL1 variant" = 1.2)) + 
    scale_alpha_manual(values = c("Healthy Control" = 0.5, 
                                 "ASXL1 variant" = 0.8)) +
    theme(axis.text.x = element_text(angle = 20, vjust = 1, hjust = 1), 
          text = element_text(size = 15)) + 
    labs(x = "", y = "Average DNAm level", title = "PRF1")
```
...
### Plot DNAm Age
plt <- meta.CC[meta.CC$Age >= 6 & meta.CC$Age <= 18, ]
ggplot(plt, aes(Age, PedBE)) + geom_smooth(method = "lm") + geom_point(aes(color = Disease_Status), size = 3) + theme_classic()
...
dim(limmaOut.bu[limmaOut.bu$adj.P.Val < 0.05, ]) # 172  
...
```{r}
load("RData/812-Buccal_Limma_test.RData")
...
### Check for BOS signature


```{r}
Awamleh_dms <- read.csv("Awamleh_BOS_ASXL1_EpiSign.csv")
Lin_dms <- read.csv("Lin_BOS_ASXL1_DM.csv")
```
...
beta <- betas(TB)
meta <- pData(TB)
meta$Array <- paste0(meta$Sentrix_ID, "_", meta$Sentrix_Position)
meth <- TB@assayData[["methylated"]]
unmeth <- TB@assayData[["unmethylated"]]
pval <- pvals(TB)
```
